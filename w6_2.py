...
class scheduler:
    ...
    def next_proc(self, pid=-1):
        #YOUR CODE
        if pid == -1:
            temp = (self.curr_proc + 1) % self.get_num_processes()
        if pid != -1:
            temp = (pid + 1) % self.get_num_processes()
        while self.proc_info[temp][PROC_STATE] == STATE_DONE \
            or self.proc_info[temp][PROC_STATE] == STATE_WAIT:
            temp = (curr+1)%self.get_num_processes()
            if temp == self.curr_proc:
                break
        self.curr_proc = temp
